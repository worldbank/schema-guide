...
The creation of a data dissemination platform is a collaborative endeavor, engaging data curators, user experience (UX) experts, designers, search engineers, and subject matter specialists with a profound understanding of both the data and the users' requirements and preferences. Inclusive in this development process should be the active participation of the users, allowing them to provide feedback that directly influences the system's design.

## Features for data users 

In order to cultivate a favorable user experience, online data catalogs must offer an intuitive and efficient interface, allowing users to effortlessly access the most pertinent datasets. To meet user expectations effectively, one should emphasize **simplicity** and **relevance**. Integrating these principles into the design of data catalogs can deliver a seamless and user-friendly experience, akin to the convenience and ease provided by internet search engines and e-commerce platforms.

Users will **search** the catalog, and **browse** entries (returned by the search, filtered, or all). Also, a catalog may **notify** users of new catalog entries that match criteria they may have provided. **Other features** sought by users include ...

Saving and sharing results: URL / API query ; export list ; social networks, etc.

In design, comply with Web Content Accessibility Guidelines (WCAG) international standard. WCAG documents explain how to make web content more accessible to people with disabilities.
...
Some users may just want to browse the content of a data catalog. The catalog's user interface should make it easy to navigate the catalog and to obtain core information on the available entries. To facilitate this "window shopping", the catalog should have the folowing features:  
