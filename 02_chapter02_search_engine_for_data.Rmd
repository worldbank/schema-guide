...
please provide one or a few paragraphs describing the convenience for users of a data catalog of having a feature that alerts them automatically when datasets of relevance to them are added to the catalog. The users must be provided with a user-friendly system to create "queries" that what would describe what makes a dataset relevant to them, applying for example filters on geographic coverage, topics covered, or other. Users would then be notified by email when relevant. An option to cancel the alerts set up by the user must be provided.
