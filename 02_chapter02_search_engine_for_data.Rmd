...
Search engines in data catalogs can be *lexical* or *semantic* (or a combination of both). A lexical search (or keyword search, or full-text search) will match literal terms entered in a query to the search engine's index and return datasets whose indexed metadata contain the terms. A semantic search aims to identify datasets that have semantically similar metadata to the query. While an ideal data catalog would offer both types of search engines (ideally combined into a single search tool), implementing semantic searchability can be complex.

(explain how semantic search works for different data types - with embeddings and vector indexing and cosine similarity - use of API)
...
What search returns: always, a list of results (list of entries) ranked by order of relevance (if none, a message "No entry match your query"). But also in some cases an answer in conversational mode (generative AI) and/or in the form of value/visulization. This will be only when the query justifies it. The system must be able to find out if such an immedate answer is appropriate. 
...
Also, some or all keywords; grouped/sequence/proximity; etc.

Advanced indexing tools like Solr and ElasticSearch provide boosting functionalities to fine-tune search engines and enhance result relevancy.
