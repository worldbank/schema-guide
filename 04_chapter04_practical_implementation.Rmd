...
Metadata conforming to the standards outlined in this Guide can be produced through two distinct methods: **utilizing a specialized metadata editor application** or **programatically** using programming languages such as R or Python.
...
On the other hand, the programmatic approach offers heightened flexibility and efficiency but necessitates a proficient understanding of R or Python. It presents numerous opportunities to automate segments of the metadata generation process and leverage advanced machine learning solutions for automatic metadata enhancement. Consequently, the programmatic alternative may be preferable for organizations equipped with expertise in R or Python. 

In this section, we provide a concise overview of how structured metadata can be generated and published using a metadata editor application, and using R or Python, respectively. Users opting for the programmatic route should have access to the comprehensive documentation of the standards' API.
...
The most recent documentation of all schemas described in the Guide is available on-line at https://ihsn.github.io/nada-api-redoc/catalog-admin/#. 
...
The righthandside frame lists all elements in a succint presentation. The middle frame provides a description of the metadata elements. 
...
The metadata elements listed in a standard can have the following properties:

   - **Optional** or **required**. When an element is **required**, the metadata will be considered invalid if they contain no information for that element. To keep the schemas flexible, very few elements are set as required. Note that it is possible for a metadata element to be `required` but have all its components (for elements that have sub-elements) declared as optional. This will be the case when at least one (but any) of the sub-element must contain information. It is also possible for an element to be declared *optional* but have one or more of its sub-elements declared `mandatory` (this means that the field is optional, but if it is used, some of its features MUST be provided.) 
   - **Simple** or **structured**. Simple elements consist of a single entry; for example, the element `title` in the standard used to document microdata is a simple element which accepts only one value. Structured elements contain sub-elements. For example, the element `producers` in the same standard contains the sub-elements `name`, `abbr`, `affiliation`, and `role`. 
...
In most cases however, controlled vocabularies are not part of the metadata standard itself as they may be specific to an organization. They may be provided and activated in templates in the Metadata Editor.
...example...
...
The easiest way to generate metadata compliant with the standards we describe in this Guide is to use a specialized Metadata Editor. A Metadata Editor provides a user-friendly and flexible interface to document data. Most metadata editors are specific to a certain standard. The World Bank developed an open source multi-standard Metadata Editor. We provide here a brief introduction to the Metadata Editor and to its use. For more detailed information, see the Metadata Editor User Guide.
