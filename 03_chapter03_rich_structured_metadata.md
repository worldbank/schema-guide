...
*Rich* metadata means metadata that is **detailed** and **comprehensive**. Rich metadata benefits both the users and the producers (or curators) of data.

### Benefits for data users

Rich metadata helps data users:
...
For the data producers (and curators), rich metadata will contribute to:
   ...
   - Provide the meta-database needed to **harmonize data collection** methods and instruments. A compelling case for such use of metadata can be found in ["The Struggle for Integration and Harmonization of Social Statistics in
...
The standards we present in this Guide have been tailored for each data type. Specialized metadata standards will foster comprehensiveness and discoverability in specialized catalog, and help build optimized data discovery suystems. But it is also critical to ensure the visibility and discoverability of the metadata in generic search engines, which are not built around the same schemas. The web makes use of its own schemas: schema.org. To ensure SEO, the specialized schemas should be mapped to it. 
